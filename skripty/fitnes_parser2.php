...
function openpage ($rowurl) {
	...
	$be->addHeaderLine("Referer", "http://www.fitnesstv.cz");   // volani odkud jsi na stranku prisel. pouzij nejakou jejich vychozi stranku.
...
echo "<channel>\n<title>Fitness TV</title>";
	

	//nastaveni bloku pro vyhledani pole polozek
	$t1 = explode('<div class="list">', $html);
    $t2 = explode('<a class="restore"', $t1[1]);
    $pom = $t2[0];
	
	
$videos = explode('<input type="hidden"', $pom);
...
//parsovani polozek
    ...
    $t1 = explode('<span class="number">', $video);
    $t2 = explode('<a class="play"', $t1[1]);
	$titulek = str_replace(array("</span>","\n\r"),array("",""),$t2[0]);
			...
			<item>
				<title>".$titulek."</title>
				<link>http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['SCRIPT_NAME'])."/fitnes_parser.php?query=".$id."</link>
			</item>\n";
    	}
