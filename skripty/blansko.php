...
$URL = "http://www.blansko.cz/media/televize-blansko/videozurnal";
if (($html = openpage($URL) ) != FALSE) {

echo "<channel>\n<title>TV Blansko</title>";
	//nastaveni bloku pro vyhledani pole polozek

	$t1 = explode('<a class="nextlev"', $html);
    $t2 = explode('class="mainpart videozurnal_width"', $t1[1]);
    $pom = $t2[0];
...
//parsovani polozek
    ...
    $titulek = $t2[0];
			...
			<item>
				<title>".$titulek."</title>
				<link>http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['SCRIPT_NAME'])."/blansko_parser.php?query=".$link.",".urlencode($titulek)."</link>
				<image>http://www.blansko.cz/img/pics/znak-velky.gif</image>
			</item>\n";
	}}		
