...
$URL = "http://www.mojevideo.sk/kategorie/";
if (($html = openpage($URL) ) != FALSE) {

echo "<channel>\n<title>Moje video SK</title>";
	

	//nastaveni bloku pro vyhledani pole polozek
$t1 = explode('<div id="blk">', $html);
    $t2 = explode('<ul id="footer">', $t1[1]);
    $pom = $t2[0];
	
$videos = explode('<td colspan="2">', $pom);
...
//parsovani polozek
    ...
    $t2 = explode('"', $t1[1]);
    $link = "http://www.mojevideo.sk".$t2[0];
	...
	IF ($titulek != "erotické videá") {
			$ItemsOut .= "
			<item>
				<title>".$titulek."</title>
				<link>http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['SCRIPT_NAME'])."/mojevideosk_parser.php?query=0,".$link."</link>
				<image>".$nahled."</image>
			</item>\n";
    	}}
