...
echo "<channel>\n<title>TV Sme.sk</title>";
	...
	$t1 = explode('<h3 style="padding-top: 0px">Relácie</h3>', $html);
    $t2 = explode('<!-- clanky stredny stlp - end -->', $t1[1]);
    $pom = $t2[0];
	
	$videos = explode('<div class="rel">', $pom);
...
//parsovani polozek
    ...
    $t1 = explode('">', $video);
    $t2 = explode('</a></div>', $t1[1]);
    $titulek = iconv("Windows-1250","UTF-8",$t2[0]);
  
  	echo "
    <item>
    <title>".$titulek."</title>
	<link>http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['SCRIPT_NAME'])."/smesk.php?query=".$link.",1</link>
	</item>
    ";
