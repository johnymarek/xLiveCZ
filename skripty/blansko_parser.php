...
//$porad = urldecode($_GET["porad"]);
$query = $_GET["query"];
if($query) {
   $queryArr = explode(',', $query);
   $porad = urldecode($queryArr[1]);
   $URL = $queryArr[0];
   }
...
echo "<channel>\n<title>TV Blansko</title>";
	...
	$t1 = explode("<h2>".$porad, $html);
    $t2 = explode('</div>', $t1[1]);
    $pom = $t2[0];

$videos = explode('<p>', $pom);
...
//parsovani polozek
	...
	$linktofinal = "http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['SCRIPT_NAME'])."/blansko_link.php?link=".$link;
