...
echo "<channel>\n<title>Hejbejse TV</title>";

if($page > 0) { ?>
<item>
<?php
$sThisFile = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME'];
$url = $sThisFile."?query=".($page-20).",".$lnk;
?>
...
<?php	
	...
	$t1 = explode('<div id="videos">', $html);
    $t2 = explode('</table>', $t1[1]);
    $pom = $t2[0];
	
	
$videos = explode('<td>', $pom);
...
//parsovani polozek
    ...
    $t2 = explode('"', $t1[1]);
    $link = "http://www.hejbejse.tv".$t2[0];
    ...
    $t2 = explode('"', $t1[1]);
    $nahled = "http://www.hejbejse.tv".$t2[0];
	
	$linktofinal = "http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['SCRIPT_NAME'])."/hejbejse_link.php?link=".$link;
