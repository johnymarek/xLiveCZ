...
if($query) {
   ...
   $captcha = $queryArr[0];
   $captchaid = $queryArr[1];
   $captchakey = $queryArr[2];
   $url = $queryArr[3];
   $ts = $queryArr[4];
   $cid = $queryArr[5];
   $sign = $queryArr[6];
   $uloztoid = $queryArr[7];
   $ULOSESSID = $queryArr[8];
   $ckey = $queryArr[9];
   $ccode = $queryArr[10];  
 }
...
curl_setopt($ch,CURLOPT_HTTPHEADER, array('Content-length: '.strlen($fields_string),'Content-Type: application/x-www-form-urlencoded','Origin: http://uloz.to','Connection: keep-alive','Cookie: uloz-to-id='.$uloztoid.'; ULOSESSID='.$ULOSESSID.';'));
...
$t1 = explode('<meta charset="', $result);
    $t2 = explode('"', $t1[1]);
   if($t2[0] == "utf-8")
   { 
     $t1 = explode('<ul class="error">', $result);
     $t2 = explode('<li>', $t1[1]);
     $t3 = explode('</li>', $t2[1]);
     $errorstr = $t3[0];
     $error = true;
